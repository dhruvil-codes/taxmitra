...
export default function Scrutiny() {
  ...
  return <div className="app-page scrutiny-page">
    ...
    {stage === "requests" && <>
      ...
      <div className="document-requests">{requests.map((r, i) => <details className="document-request" key={r.id} open={i===0}><summary><span>{String(i+1).padStart(2,"0")}</span><strong>{text(r.plain_language_explanation,locale) || r.response_section}</strong><i>{locale === "hi" ? "खोलें" : "OPEN"}</i></summary><div className="document-request-body">{r.required_evidence.length>0&&<div className="request-evidence"><b>{locale === "hi" ? "आपको यह चाहिए" : "YOU WILL NEED"}</b><ul>{r.required_evidence.map((e,j)=><li key={j}>{text(e,locale)}</li>)}</ul></div>}<details className="original-source"><summary>{locale === "hi" ? "मूल नोटिस अनुरोध देखें" : "View original notice request"}</summary><div className="official-wording"><b>{locale === "hi" ? "अधिकारी के मूल शब्द" : "OFFICIAL REQUEST WORDING"}</b><blockquote>{r.original_text}</blockquote></div><div className="scrutiny-explain"><div><b>{locale === "hi" ? "यह क्यों माँगा गया" : "WHY REQUIRED"}</b><p>{text(r.why_required,locale)}</p></div></div></details><div className="request-grounding"><span>{r.response_section.toUpperCase()}</span><span>{Math.round(r.confidence*100)}% {locale === "hi" ? "विश्वास" : "CONFIDENCE"}</span><span>{r.grounding.method} {Math.round(r.grounding.confidence*100)}%</span></div>{r.warnings.map((warning,j)=><p className="request-warning" key={j}>{warning}</p>)}{r.citations.map(c=><a key={c.id} className="scrutiny-source" href={c.official_url} target="_blank" rel="noreferrer">{c.source_name} · {c.section} ↗</a>)}</div></details>)}</div>
