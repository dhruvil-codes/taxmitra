...
export default function Scrutiny() {
  ...
  return <div className="app-page scrutiny-page">
    ...
    {stage === "final" && result?.official_step && <><p className="app-eyebrow">[ {t("scrutiny.officialHandoff")} / 04 ]</p><h1 className="app-title">{t("j.finalTitle")}</h1><Card className="app-dots"><p className="app-section-label">[ {t("scrutiny.whatToDo")} ]</p><h2 className="text-2xl font-medium">{text(result.official_step.label,locale)}</h2><p className="deadline-inline">{result.deadline?.due_date}</p></Card><div className="value-summary"><p className="app-section-label">[ {t("scrutiny.valueSummary")} ]</p><ul className="value-list"><li>✓ {t("scrutiny.valueRead")}</li><li>✓ {t("scrutiny.valueIdentified")}</li><li>✓ {t("scrutiny.valueBroke", {count: String(requests.length)})}</li><li>✓ {t("scrutiny.valueChecked")}</li><li>✓ {t("scrutiny.valueVerification")}</li><li>✓ {t("scrutiny.valueDraft")}</li><li>✓ {t("scrutiny.valueGuided")}</li></ul></div><div className="notice-boundary"><p className="app-section-label">[ {t("scrutiny.readyStep")} ]</p><p className="app-body font-medium">{t("scrutiny.readyText")}</p><p className="app-body mt-2">{t("j.finalBoundary")}</p><p className="app-body mt-2">{t("j.finalInstruction")}</p></div><div className="final-actions"><a className="app-primary" href={OFFICIAL_EFILING_PORTAL_URL} target="_blank" rel="noopener noreferrer">{t("j.continuePortal")}</a><button onClick={downloadDraft}>{t("scrutiny.downloadDraft")}</button><button onClick={async()=>{await navigator.clipboard.writeText(draft);setFinalCopied(true);setTimeout(()=>setFinalCopied(false),2000)}}>{finalCopied ? t("scrutiny.copied") : t("scrutiny.copyDraft")}</button></div><button className="app-back" onClick={()=>setStage("review")}>← {t("j.back")}</button></>}
