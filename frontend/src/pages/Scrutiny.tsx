...
export default function Scrutiny() {
  ...
  return <div className="app-page scrutiny-page">
    ...
    {stage === "requests" && <>
      <p className="app-eyebrow">[ TM / SCRUTINY / REQUESTS ]</p><h1 className="app-title">{locale === "hi" ? "अपने नोटिस में क्या मांगा गया है, जांचें" : "Review what your notice is asking for"}</h1>
      ...
      <p className="app-lead">{locale === "hi" ? `हमने आपके नोटिस में ${requests.length} अनुरोध पाए हैं। प्रत्येक की समीक्षा करें, अपने रिकॉर्ड जांचें, और पुष्टि करें।` : `We found ${requests.length} requests in your notice. Review each one, check the records you have, and confirm before Tax Mitra guides you further.`}</p>
      <p className="request-count">{String(requests.length).padStart(2,"0")} {locale === "hi" ? "अनुरोध मिले" : "requests found"}</p>
      <div className="document-requests">{requests.map((r, i) => <details className="document-request" key={r.id} open={i===0}><summary><span>{String(i+1).padStart(2,"0")}</span><strong>{text(r.plain_language_explanation,locale) || r.response_section}</strong><i>{locale === "hi" ? "खोलें" : "OPEN"}</i></summary><div className="document-request-body"><p className="request-label">{locale === "hi" ? "नोटिस में क्या पूछा गया है" : "WHAT THE NOTICE IS ASKING"}</p><p className="request-explanation">{text(r.plain_language_explanation,locale)}</p>{r.required_evidence.length>0&&<><p className="request-label">{locale === "hi" ? "आपको क्या चाहिए" : "WHAT YOU MAY NEED"}</p><ul className="evidence-list-simple">{r.required_evidence.map((e,j)=><li key={j}>{text(e,locale)}</li>)}</ul></>}<details className="original-source"><summary>{locale === "hi" ? "मूल नोटिस अनुरोध देखें →" : "View original notice request →"}</summary><div className="official-wording"><b>{locale === "hi" ? "अधिकारी के मूल शब्द" : "OFFICIAL REQUEST WORDING"}</b><blockquote>{r.original_text}</blockquote></div></details><details className="verification-details"><summary>{locale === "hi" ? "Tax Mitra ने यह कैसे समझा →" : "How Tax Mitra understood this →"}</summary><div className="verification-content"><div className="verification-row"><span>{locale === "hi" ? "प्रतिक्रिया खंड" : "Response section"}</span><b>{r.response_section}</b></div><div className="verification-row"><span>{locale === "hi" ? "निष्कर्ष विश्वास" : "Extraction confidence"}</span><b>{Math.round(r.confidence*100)}%</b></div><div className="verification-row"><span>{locale === "hi" ? "आधार विश्वास" : "Grounding confidence"}</span><b>{r.grounding.method} {Math.round(r.grounding.confidence*100)}%</b></div>{r.why_required&&<div className="verification-row"><span>{locale === "hi" ? "यह क्यों माँगा गया" : "Why required"}</span><p>{text(r.why_required,locale)}</p></div>}{r.warnings.length>0&&<div className="verification-warnings">{r.warnings.map((warning,j)=><p key={j}>{warning}</p>)}</div>}{r.citations.length>0&&<div className="verification-citations"><p className="verification-label">{locale === "hi" ? "स्रोत" : "Sources"}</p>{r.citations.map(c=><a key={c.id} href={c.official_url} target="_blank" rel="noreferrer">{c.source_name} · {c.section} ↗</a>)}</div>}</div></details></div></details>)}</div>
    ...
    {stage === "draft" && <><p className="app-eyebrow">[ RESPONSE PACKAGE / DRAFT ]</p><h1 className="app-title">{t("j.draftTitle")}</h1><p className="app-lead mb-5">{t("j.draftSub")}</p><textarea className="scrutiny-draft" rows={20} value={draft} onChange={e=>setDraft(e.target.value)} onBlur={()=>store.setDraft(id,draft)} /><div className="flex flex-wrap items-center gap-3"><PrimaryButton onClick={()=>{store.setDraft(id,draft);setStage("review")}}>{t("j.acceptDraft")} →</PrimaryButton><button onClick={downloadDraft}>{locale === "hi" ? "डाउनलोड मसौदा" : "Download draft"}</button><button onClick={async()=>{await navigator.clipboard.writeText(draft);setCopied(true);setTimeout(()=>setCopied(false),2000)}}>{copied ? (locale === "hi" ? "कॉपी किया गया!" : "Copied!") : (locale === "hi" ? "कॉपी मसौदा" : "Copy draft")}</button><button className="app-back !mt-0" onClick={()=>setStage("result")}>← {t("j.back")}</button></div></>}
    ...
    {stage === "final" && result?.official_step && <><p className="app-eyebrow">[ OFFICIAL HANDOFF / 04 ]</p><h1 className="app-title">{t("j.finalTitle")}</h1><Card className="app-dots"><p className="app-section-label">[ WHAT TO DO ]</p><h2 className="text-2xl font-medium">{text(result.official_step.label,locale)}</h2><p className="deadline-inline">{result.deadline?.due_date}</p></Card><div className="value-summary"><p className="app-section-label">[ HERE'S WHAT TAX MITRA HELPED YOU WITH ]</p><ul className="value-list"><li>✓ {locale === "hi" ? "आपके नोटिस को समझा" : "Read your notice"}</li><li>✓ {locale === "hi" ? "क्या मांगा गया पहचाना" : "Identified what information was requested"}</li><li>✓ {locale === "hi" ? `${requests.length} अलग-अलग अनुरोधों में तोड़ा` : `Broke the notice into ${requests.length} individual requests`}</li><li>✓ {locale === "hi" ? "अपने रिकॉर्ड जांचने में मदद की" : "Helped you check what records you have"}</li><li>✓ {locale === "hi" ? "सत्यापन की आवश्यकता पहचानी" : "Identified items that may need verification"}</li><li>✓ {locale === "hi" ? "प्रतिक्रिया मसौदा तैयार किया" : "Prepared a response draft"}</li><li>✓ {locale === "hi" ? "आधिकारिक e-Filing पोर्टल का मार्गदर्शन किया" : "Guided you to the official e-Filing portal"}</li></ul></div><div className="notice-boundary"><p className="app-section-label">[ YOU'RE READY FOR THE NEXT STEP ]</p><p className="app-body font-medium">{locale === "hi" ? "आप आगे बढ़ने के लिए तैयार हैं।" : "You're ready for the next step."}</p><p className="app-body mt-2">{t("j.finalBoundary")}</p><p className="app-body mt-2">{t("j.finalInstruction")}</p></div><div className="final-actions"><a className="app-primary" href={OFFICIAL_EFILING_PORTAL_URL} target="_blank" rel="noopener noreferrer">{t("j.continuePortal")}</a><button onClick={downloadDraft}>{locale === "hi" ? "डाउनलोड मसौदा" : "Download draft"}</button><button onClick={async()=>{await navigator.clipboard.writeText(draft);setFinalCopied(true);setTimeout(()=>setFinalCopied(false),2000)}}>{finalCopied ? (locale === "hi" ? "कॉपी किया गया" : "Copied!") : (locale === "hi" ? "कॉपी मसौदा" : "Copy draft")}</button></div><button className="app-back" onClick={()=>setStage("review")}>← {t("j.back")}</button></>}
