...
export default function Upload() {
  ...
  const confirm = async (confirmed: boolean) => {
    ...
    try {
      const response = await api.confirmExtraction(result.extraction_id, result.fingerprint, confirmed, corrections);
  ...
  return <WorkflowLayout currentStep={0}>
    ...
    <GuidedInteraction step={locale === "hi" ? "समझें · चरण 1 / 6" : "Understand · Step 1 of 6"} title={locale === "hi" ? "अपना कर नोटिस अपलोड करें।" : "Upload your tax notice."} instruction={locale === "hi" ? "पहले अपना नोटिस चुनें। इसके बाद Tax Mitra उसे पढ़कर आपके स��मने जाँच के लिए रखेगा।" : "Choose your notice first. Tax Mitra will extract it and show you the result to check."} />
    ...
    {result?.supported && <section aria-labelledby="review-title">
      ...
      <div className="human-check"><p className="app-section-label">[ HUMAN CHECK / 02 ]</p><p className="app-body">{locale === "hi" ? "प���ष्टि तभी करें जब सूची PDF से पूरी तरह मेल खाती हो। PDF bytes memory में process होते हैं, store या log नहीं होते; session 30 मिनट में समाप्त होता है।" : "Confirm only if this list matches the PDF. PDF bytes are processed in memory and are not stored or logged; the session expires after 30 minutes."}</p></div>
